    ...
    <div class="content-header mb-3">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                ...
                <small class="text-muted">أدخل البيانات الأساسية ثم احفظ. يمكنك إضافة تصنيف/أب من نفس الصفحة.</small>
            </div>
        ...
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge bg-light text-muted">Create</span>
                </div>

                <div class="card-body">
                    <form action="{{ route('items.store') }}" method="POST" id="itemForm" enctype="multipart/form-data" novalidate>
                        @csrf
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="text" name="items_code" value="{{ $nextCode }}" class="form-control @error('items_code') is-invalid @enderror" readonly>
                            ...
                            <div class="col-md-4 mb-3">
                                <label class="form-label d-flex justify-content-between align-items-center">
                                    ...
                                    <button type="button" class="btn btn-sm btn-outline-secondary btn-icon" id="btnGenBarcodeMain">
                                        <i class="ti ti-refresh"></i> توليد
                                    </button>
                                </label>
                                <input type="text" name="barcode" id="barcode_input" value="{{ old('barcode') }}"
                                       class="form-control @error('barcode') is-invalid @enderror" required inputmode="numeric">
                                @error('barcode')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                <small class="text-muted form-hint">12 رقم ويبدأ بـ 20 (سيتم التحقق أيضًا عند الحفظ).</small>
                            </div>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="type" class="form-select @error('type') is-invalid @enderror" required>
                                    ...
                                    <option value="store" @selected(old('type') === 'store')>مخزني</option>
                                    <option value="consumption" @selected(old('type') === 'consumption')>استهلاكي</option>
                                    <option value="custody" @selected(old('type') === 'custody')>عهدة</option>
                                </select>
                                ...
                                <div class="d-flex flex-wrap align-items-center gap-3">
                                    ...
                                    <div class="flex-grow-1">
                                        ...
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <button type="button" class="btn btn-sm btn-light" id="btnClearImage">إزالة</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="item_category_id" id="category_select" class="form-select select2 @error('item_category_id') is-invalid @enderror" required>
                                    <option value="">-- اختر التصنيف --</option>
                                    @foreach($categories as $cat)
                                        <option value="{{ $cat->id }}" @selected(old('item_category_id') == $cat->id)>
                                            {{ $cat->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="item_id" id="parent_item_select" class="form-select select2 @error('item_id') is-invalid @enderror">
                                    ...
                                    @foreach($parents as $p)
                                        <option value="{{ $p->id }}" @selected(old('item_id') == $p->id)>
                                            {{ $p->name }} ({{ $p->items_code }})
                                        </option>
                                    @endforeach
                                </select>
                                @error('item_id')<div class="invalid-feedback d-block">{{ $message }}</div>@enderror
                                <small class="text-muted form-hint">استخدم الأب لتجميع الأصناف تحت صنف رئيسي.</small>
                            </div>

                            {{-- Unit --}}
                            <div class="col-md-6 mb-3">
                                <label class="form-label">وحدة القياس الأساسية <span class="text-danger">*</span></label>
                                <input type="text" name="unit_id" value="{{ old('unit_id') }}"
                            ...
                            <div class="col-md-6 mb-3 d-flex align-items-end">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="status" value="1" id="status" @checked(old('status'))>
                            ...
                            <div class="col-12 mb-2">
                                ...
                                <small class="text-muted form-hint">يجب إدخال جميع الأسعار (حسب جدول قاعدة البيانات).</small>
                            </div>

                            <div class="col-md-4 mb-3">
                                <label class="form-label">السعر القطاعي (وحدة الأب) <span class="text-danger">*</span></label>
                                <input type="number" step="0.01" min="0" name="price"
                                       value="{{ old('price') }}" class="form-control money @error('price') is-invalid @enderror" required>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" step="0.01" min="0" name="nos_egomania_price"
                                       value="{{ old('nos_egomania_price') }}" class="form-control money @error('nos_egomania_price') is-invalid @enderror" required>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" step="0.01" min="0" name="egomania_price"
                                       value="{{ old('egomania_price') }}" class="form-control money @error('egomania_price') is-invalid @enderror" required>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" step="0.01" min="0" name="price_retail"
                                       value="{{ old('price_retail') }}" class="form-control money @error('price_retail') is-invalid @enderror" required>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" step="0.01" min="0" name="nos_gomla_price_retail"
                                       value="{{ old('nos_gomla_price_retail') }}" class="form-control money @error('nos_gomla_price_retail') is-invalid @enderror" required>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" step="0.01" min="0" name="gomla_price_retail"
                                       value="{{ old('gomla_price_retail') }}" class="form-control money @error('gomla_price_retail') is-invalid @enderror" required>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="does_has_retail_unit"
                                           value="1" id="does_has_retail_unit" @checked(old('does_has_retail_unit'))>
                            ...
                            <div class="col-md-6 mb-3" id="retail_unit_wrap">
                                ...
                                <input type="text" name="retail_unit" id="retail_unit" value="{{ old('retail_unit') }}"
                            ...
                            <div class="col-md-6 mb-3" id="retail_ratio_wrap">
                                ...
                                <input type="number" step="0.01" min="1" name="retail_uom_quintToParent" id="retail_uom_quintToParent"
                                       value="{{ old('retail_uom_quintToParent') }}" class="form-control @error('retail_uom_quintToParent') is-invalid @enderror">
...
@section('js')
    ...
    <script>
        ...
        // Modals focus
        $('#createCategoryModal').on('shown.bs.modal', function () { $('#new_category_name').trigger('focus'); });
        $('#createParentItemModal').on('shown.bs.modal', function () {
            if (!$('#new_parent_barcode').val()) $('#new_parent_barcode').val(generateBarcode());
            $('#new_parent_code').trigger('focus');
        });

        // Main barcode generate
        document.getElementById('btnGenBarcodeMain')?.addEventListener('click', function () {
            document.getElementById('barcode_input').value = generateBarcode();
        });

        // ===== Image Preview =====
        const defaultImg = "{{ asset('images/no-image.png') }}";
        ...
        document.getElementById('btnClearImage')?.addEventListener('click', function () {
            ...
            document.getElementById('imagePreview').src = defaultImg;
        });
        ...
        function validateForm(){
            ...
            if(retailEnabled){
                ...
                // ensure cleared (UI + request)
                if(retailUnitEl) retailUnitEl.value = '';
        ...
        // prevent double submit + validate before controller
        $('#itemForm').on('submit', function(e){
            ...
            btn.html('<i class="ti ti-loader"></i> جارٍ الحفظ...');
        });

        // ===== Modal validation helpers (كما هي) =====
        ...
        // clear invalid state on typing in modals
