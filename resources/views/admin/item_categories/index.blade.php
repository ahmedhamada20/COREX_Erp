        ...
        <div class="col-12">
            ...
            <div class="card table-card">
                <div class="card-header">
                    <h6 class="mb-0">قائمة فئات الأصناف</h6>
                </div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <div class="btn-group btn-group-sm" role="group">
                                                <a href="{{ route('item_categories.edit', $item->id) }}"
                                                   class="btn btn-sm btn-success">
                                                    <i class="fa fa-edit"></i>
                                                </a>

{{--                                                <a href="{{ route('item_categories.show', $item->id) }}"--}}
{{--                                                   class="btn btn-sm btn-info">--}}
{{--                                                    <i class="fa fa-eye"></i>--}}
{{--                                                </a>--}}

                                                <form action="{{ route('item_categories.destroy', $item->id) }}"
                                                      method="POST"
...
@section('js')
    ...
    <script>
        $(document).on('change', '.toggle-status', function () {
            ...
            $.ajax({
                url: "{{ url('/item_categories/toggle-status') }}/" + id,
                type: 'POST',
                data: {
                    _token: "{{ csrf_token() }}"
                },
                success: function (res) {
                    toastr.success("تم تحديث الحالة إلى: " + (res.status_text ?? 'تم'), "نجاح");
                },
                error: function () {
