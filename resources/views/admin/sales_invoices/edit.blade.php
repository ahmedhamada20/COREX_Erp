{{-- resources/views/admin/sales_invoices/edit.blade.php --}}
...
@section('content')
    @php
        // ====== Safe defaults (لو الكنترولر مش باعتهم) ======
        ...
        // ====== Pre-fill payments from invoice (لو invoice->payments موجودة) ======
        $invoice->loadMissing(['customer','items.item','payments']);

        $invPayCash   = (float) ($invoice->payments?->where('method','cash')->sum('amount') ?? 0);
        $invPayCard   = (float) ($invoice->payments?->where('method','card')->sum('amount') ?? 0);
        $invPayWallet = (float) ($invoice->payments?->where('method','wallet')->sum('amount') ?? 0);

        $defaultPayCash   = (float) old('payment.cash', $invPayCash);
        $defaultPayCard   = (float) old('payment.card', $invPayCard);
        $defaultPayWallet = (float) old('payment.wallet', $invPayWallet);

        // ====== Global UI defaults ======
        $defaultGlobalDiscType  = old('global_discount_type', 'amount');
        $defaultGlobalDiscValue = (float) old('global_discount_value', 0);

        // VAT default for new lines
        $defaultVatRate = (float) old('global_vat_rate', 14);

        // ====== IMPORTANT: Prepare items array in PHP to avoid Blade ParseError ======
        $itemsData = $invoice->items->map(function ($it) {
            ...
            // لو اسم الخصم عندك مختلف غيّره هنا
            $discAmount = (float) ($it->discount ?? 0);

            $base = max(0, $gross - $discAmount);

            // لو اسم vat عندك مختلف غيّره هنا
            $vatAmount = (float) ($it->vat ?? 0);

            $vatRate = 0.0;
            if ($base > 0.0001 && $vatAmount > 0) {
                $vatRate = round(($vatAmount / $base) * 100, 4);
            }
            ...
            return [
                ...
                'qty'            => round($qty, 4),
                'price'          => round($price, 4),
                'discount_value' => round($discAmount, 4),
                'discount_type'  => 'amount',
                'vat_rate'       => round($vatRate, 4),
                'cost_price'     => round((float) ($it->cost_price ?? 0), 4),
            ];
        })->values()->toArray();
    @endphp
        ...
        <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-2">
            <div>
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    <h4 class="mb-0">تعديل فاتورة مبيعات #{{ $invoice->invoice_code }}</h4>
                    <span class="badge-soft">{{ ucfirst($invoice->status) }}</span>
            ...
            <div class="d-flex gap-2">
                <a href="{{ route('sales_invoices.show', $invoice->id) }}" class="btn btn-light pos-btn">
    ...
    <form id="posForm" method="POST" action="{{ route('sales_invoices.update', $invoice->id) }}">
        @csrf
        @method('PUT')
        ...
        {{-- Totals --}}
        <input type="hidden" name="subtotal"         id="subtotal_input"  value="{{ old('subtotal', (float)$invoice->subtotal) }}">
        <input type="hidden" name="discount_amount"  id="discount_input"  value="{{ old('discount_amount', (float)$invoice->discount_amount) }}">
        <input type="hidden" name="vat_amount"       id="vat_input"       value="{{ old('vat_amount', (float)$invoice->vat_amount) }}">
        <input type="hidden" name="total"            id="total_input"     value="{{ old('total', (float)$invoice->total) }}">
        <input type="hidden" name="remaining_amount" id="remaining_input" value="{{ old('remaining_amount', (float)$invoice->remaining_amount) }}">
        ...
        <input type="hidden" name="payment[cash]"        id="pay_cash" value="{{ $defaultPayCash }}">
        <input type="hidden" name="payment[card]"        id="pay_card" value="{{ $defaultPayCard }}">
        <input type="hidden" name="payment[wallet]"      id="pay_wallet" value="{{ $defaultPayWallet }}">
        <input type="hidden" name="payment[treasury_id]" id="pay_treasury_id" value="{{ old('payment.treasury_id', $openShiftTreasuryId ?? ($invoice->payments->first()->treasury_id ?? '')) }}">
        <input type="hidden" name="payment[terminal_id]" id="pay_terminal_id" value="{{ old('payment.terminal_id', ($invoice->payments->first()->terminal_id ?? '')) }}">
                            ...
                            <div class="row g-2">
                                <div class="col-md-3">
                                    ...
                                    <input type="date" name="invoice_date" class="form-control"
                                           value="{{ old('invoice_date', $invoice->invoice_date ?? $today) }}" required style="border-radius:14px;height:40px;">
                                </div>

                                <div class="col-md-3">
                                    <label class="form-label">الاستحقاق</label>
                                    <input type="date" name="due_date" id="due_date" class="form-control"
                                           value="{{ old('due_date', $invoice->due_date) }}" style="border-radius:14px;height:40px;">
                                </div>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select name="payment_type" id="payment_type" class="form-select" required style="border-radius:14px;height:40px;">
                                        <option value="cash" @selected((old('payment_type') ?? $invoice->payment_type) === 'cash')>كاش</option>
                                        <option value="credit" @selected((old('payment_type') ?? $invoice->payment_type) === 'credit')>آجل</option>
                                    </select>
                                </div>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select name="warehouse_id" id="warehouse_id" class="form-select" required style="border-radius:14px;height:40px;">
                                        ...
                                        @foreach($warehouses as $w)
                                            <option value="{{ $w->id }}" @selected((string)old('warehouse_id', $invoice->warehouse_id ?? '') === (string)$w->id)>
                                                {{ $w->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-12">
                                    ...
                                    <select name="customer_id" id="customer_id" class="form-select"
                                        ...
                                        <option value=""></option>
                                        <option value="{{ $invoice->customer_id }}" selected>{{ $invoice->customer->name ?? 'Customer' }}</option>
                                    </select>
                                    <input type="hidden" name="customer_text" id="customer_text" value="{{ old('customer_text', $invoice->customer->name ?? '') }}">
                                ...
                                <div class="col-12">
                                    ...
                                    <textarea name="notes" class="form-control" rows="2" style="border-radius:14px;"
                                              placeholder="ملاحظات داخلية...">{{ old('notes', $invoice->notes) }}</textarea>
                                </div>
    ...
    {{-- Template Row (JS will clone) --}}
    @php
        // بنكتب template كسطر واحد في HTML (JS بيحطه في cartBody)
    @endphp
            ...
            <div class="modal-content" style="border-radius:18px; overflow:hidden;">
                ...
                <div class="modal-body">
                    <div class="row g-2">
                        ...
                        <div class="col-md-6">
                            <label class="form-label">الماكينة/Terminal (اختياري)</label>
                            <select id="pm_terminal" class="form-select" style="border-radius:14px;height:40px;">
                                <option value="">—</option>
                                @foreach($terminals as $trm)
                                    <option value="{{ $trm->id }}">{{ $trm->name }}</option>
                                @endforeach
                            </select>
                            <div class="hint">يتسجل مع دفعة الفيزا/المحفظة لو عندك.</div>
                        </div>
...
@section('js')
    @php
        $invoiceDataArray = [
            ...
            'subtotal'     => (float) old('subtotal', (float) $invoice->subtotal),
            'discount'     => (float) old('discount_amount', (float) $invoice->discount_amount),
            'vat'          => (float) old('vat_amount', (float) $invoice->vat_amount),
            'total'        => (float) old('total', (float) $invoice->total),
        ];
    @endphp
    ...
    <script>
        ...
        function initItemSelect2($el){
            ...
            $el.on('select2:select', function(e){
                ...
                if (subParts.length) $row.find('.sub').text(subParts.join(' — '));
        ...
        function collectCurrentSale(){
            ...
            $('#cartBody .cart-row').each(function(){
                ...
                rows.push({
                    ...
                    cost_price: num($row.find('.cost_price').val()),
        ...
        function loadSaleToUI(sale){
            ...
            if (sale.rows && sale.rows.length) {
                sale.rows.forEach(r => {
                    ...
                    $row.find('.name').text(r.item_text || 'صنف');
                    ...
                    $row.find('.cost_price').val(Number(r.cost_price || 0).toFixed(4));
        ...
        function addToCartByData(id, text, price, qty, stock, offerBuy, offerGet, reorderLevel){
            ...
            $row.find('input[name$="[item_text]"]').val(text || '');
            $row.find('.name').text(text || 'صنف');
        ...
        const invoiceData = @json($invoiceDataArray);

        const itemsData = @json($itemsData);

        function mountInvoiceItems(){
            $('#cartBody').html('');
            ...
            if (itemsData && itemsData.length) {
                itemsData.forEach((it) => {
                    ...
                    const opt = new Option(it.item_text || 'Item', String(it.item_id), true, true);
                    $sel.append(opt).trigger('change.select2');

                    $row.find('input[name$="[item_text]"]').val(it.item_text || '');
                    $row.find('.name').text(it.item_text || 'صنف');

                    $row.find('.qty').val(Number(it.qty || 1).toFixed(4));
                    $row.find('.price').val(Number(it.price || 0).toFixed(4));
                    $row.find('.discount_value').val(Number(it.discount_value || 0).toFixed(4));
                    $row.find('.discount_type').val(it.discount_type || 'amount');
                    $row.find('.vat_rate').val(Number(it.vat_rate || 0).toFixed(4));
                    $row.find('.cost_price').val(Number(it.cost_price || 0).toFixed(4));
        ...
        $(function(){
            ...
            // set initial payments into modal helpers
            calcAll();
        ...
        // Prevent submit if negative stock OR missing cost_price
        $('#posForm').on('submit', function(e){
            ...
            if (hasMissingCostPrice) {
                e.preventDefault();
                toastr.error('جميع الأصناف يجب أن تملك سعر تكلفة أكبر من صفر (من Edit السطر).');
                return false;
            }
